...
[nagi.yaml](../reference/project.md) の `backend.type` で、ストレージバックエンドを切り替えられます。

リモートバックエンドを選択すると、[`nagi serve`](../reference/cli.md#serve) がリモートサーバーで動いている場合には、クライアント環境から [`nagi status`](../reference/cli.md#status) や [`nagi serve resume`](../reference/cli.md#serve) を実行できます。

## Caches

Evaluate の結果を Asset・期待状態ごとにファイルとして保存します。[`nagi status`](../reference/cli.md#status) はこのキャッシュを読み取るため、評価は実行しません。`nagi serve` では期待状態ごとの `evaluateCacheTtl` に基づき、TTL 内であればキャッシュ済みの結果を再利用してクエリの実行をスキップします。
...
!!! tip
    すべてのロックは TTL（有効期限）付きで、プロセスが異常終了した場合でも期限切れにより自動で解放されます。TTL は [`nagi.yaml`](../reference/project.md) の `lockTtlSeconds` で変更できます（デフォルト: 3600秒）。

ファイル名は [kind: Asset](../reference/resources/asset.md) の `metadata.name` に対応します。
...
Guardrails が sync を停止した Asset のフラグです。停止理由、停止時刻、起動元となった sync の実行 ID を含みます。[`nagi serve resume`](../reference/cli.md#serve-resume) で解除します。
...
`logs.db` は SQLite データベースで、evaluate と sync の実行履歴を記録します。[`nagi init`](../reference/cli.md#init) でスキーマが初期化されます。既にデータベースが存在する場合は既存のデータを保持します。[`nagi status`](../reference/cli.md#status) はこのデータベースから直近の実行結果を読み取ります。
...
`logs.db` は [`nagi serve`](../reference/cli.md#serve) を実行した環境のファイルシステムにつくられます。

`logs.db` のデータは [`nagi export`](../reference/cli.md#export) でデータウェアハウスへエクスポートできます。設定方法や仕組みの解説は [Export](./export.md) を参照してください。

## Watermarks

[`nagi export`](../reference/cli.md#export) が データウェアハウスにエクスポート済みの位置を記録するファイルです。テーブルごとにウォーターマーク（最後にエクスポートした `rowid`）を保持し、差分転送を実現します。詳細は [Export](./export.md) を参照してください。

...
Guardrails は、Sync による状態悪化や繰り返しの失敗から被害が拡大することを防ぐ仕組みです。

| 状態 | 説明 |
| --- | --- |
| Cooldown | Sync 失敗後、次の Sync 起動が時間ベースで抑制される |
| Suspend | Asset の Sync が停止される。Evaluate は継続する |

#### Cooldown transitions

| イベント | 遷移 |
| --- | --- |
| Sync 1回目失敗 | Cooldown 開始（初期待機時間の経過後に次の Sync が可能） |
| Sync 2回目失敗 | Cooldown 延長（待機時間が倍増） |
| Cooldown タイマー経過 | Cooldown 解除 |
| Sync 成功 | Cooldown 解除（カウンタもリセット） |
| 連続失敗が上限に到達 | Suspend に遷移 |

#### Suspend transitions

| イベント | 遷移 |
| --- | --- |
| Sync が連続で上限回数失敗 | Suspend 開始（自動） |
| Sync 後に Ready な条件の数が減少 | Suspend 開始（自動） |
| `nagi serve halt` 実行 | 全 Asset が Suspend（手動） |
| Asset が Ready に復帰 | Suspend 解除（自動） |
| `nagi serve resume` 実行 | Suspend 解除（手動） |
...
- EvaluateFailed — Evaluate が失敗した場合

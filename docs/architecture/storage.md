...
Evaluate の結果を Asset・条件ごとにファイルとして保存します。[`nagi status`](../reference/cli.md#status) はこのキャッシュを読み取るため、評価は実行しません。`nagi serve` では条件ごとの `evaluateCacheTtl` に基づき、TTL 内であればキャッシュ済みの結果を再利用してクエリの実行をスキップします。

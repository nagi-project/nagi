...
| watermarks/ | DWH エクスポートのウォーターマーク | `~/.nagi/watermarks/` | `~/.nagi/watermarks/` |
| source_stats/ | Source テーブルの統計値キャッシュ | `~/.nagi/source_stats/` | `{bucket}/source_stats/` |
...
`logs.db` のデータは [`nagi export`](../cli.md#export) でリモート DWH にエクスポートできます。エクスポートの設定は [`nagi.yaml`](../configurations/project.md) の `export` を参照してください。

## Watermarks

[`nagi export`](../cli.md#export) が DWH にエクスポート済みの位置を記録するファイルです。テーブルごとにウォーターマーク（最後にエクスポートした `rowid`）を保持し、差分転送を実現します。
...
Source テーブルの統計値（行数・バイト数）をキャッシュします。前回の統計値と変化がない場合にデータウェアハウスへのクエリを省略する最適化に使用します。

...
| --- | --- |
| Asset | evaluate と sync の対象。`onDrift` で条件と収束操作のペアを定義する |
| Source | Asset が依存する上流データ。条件を持つ場合は sync の前提条件として機能する |
| Connection | Source が参照するデータウェアハウスへの接続情報。evaluate 時のクエリ実行に使用する |

...
内部アーキテクチャの詳細は [Serve](./architecture/serve.md) を参照してください。
...
- sync 後の re-evaluate（収束結果の確認）

上流 Asset が Drifted → Ready に遷移した場合は、下流の evaluate をスキップして直接 sync を起動します。また、上流が Drifted の間は下流の evaluate はブロックされます。詳細は [Serve: Upstream State Change](./architecture/serve.md#upstream-state-change) を参照してください。

各起動条件の設定方法は [kind: Asset](./configurations/resources/asset.md) を参照してください。

...
use crate::runtime::compile::GraphEdge;
use crate::runtime::evaluate::{AssetEvalResult, EvaluateError};
        ...
        let (ready, ready_count) = match result {
            Ok(r) => {
                let count = r
                    .conditions
                    .iter()
                    .filter(|c| c.status == crate::runtime::evaluate::ConditionStatus::Ready)
                    .count();
                (r.ready, count)
            }
            Err(_) => (false, 0),
...
mod tests {
    ...
    fn ready_condition(name: &str) -> crate::runtime::evaluate::ConditionResult {
        crate::runtime::evaluate::ConditionResult {
            ...
            status: crate::runtime::evaluate::ConditionStatus::Ready,
        }
    }

    fn not_ready_condition(name: &str) -> crate::runtime::evaluate::ConditionResult {
        crate::runtime::evaluate::ConditionResult {
            condition_name: name.to_string(),
            condition_type: "test".to_string(),
            status: crate::runtime::evaluate::ConditionStatus::Drifted {
    ...
    fn on_sync_complete_success_resets_guardrail() {
        ...
        state.syncing.insert("a".to_string());
        let sync_result = Ok(crate::runtime::sync::SyncExecutionResult {

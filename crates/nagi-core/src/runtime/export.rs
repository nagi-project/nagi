...
pub enum ExportError {
    ...
    #[error("log error: {0}")]
    Log(#[from] crate::runtime::log::LogError),

    #[error("unknown table: {0}")]
    UnknownTable(String),

    #[error("connection error: {0}")]
    Connection(#[from] crate::runtime::kind::connection::ConnectionError),
}
...
pub fn resolve_export_connection(
    ...
    let resolved =
        crate::runtime::kind::connection::resolve_connection_by_name(connection_name, &connections)
            .map_err(|e| ExportError::Io(std::io::Error::other(e.to_string())))?;
...
mod tests {
    ...
    fn should_export_true_when_no_marker() {
        let dir = tempfile::tempdir().unwrap();
        let interval: crate::runtime::duration::Duration = serde_yaml::from_str("30m").unwrap();
